...
interface ReportLearning {
  ...
  type QAReportLearningProps = {
    selectedDepartmentId: number;
    selectedYear: string;
  };

  export default function QALearning({selectedDepartmentId,selectedYear }: QAReportLearningProps) {
    ...
    if (session?.user?.name) user = JSON.parse(session?.user?.name as string);
     ...
     //Report
  const [learningReports, setLearningReports] = useState<ReportLearning[]>(
    []
  );
  const [primaryLearningReports, setPrimaryLearningReports] = useState<
    ReportLearning[]
  >([]);
  const allLearningReports = [...primaryLearningReports, ...learningReports];
  ...
  useEffect(() => {
    const getPrimaryReports = async (selectedDepartmentId: number) => {
      try {
        ...
        const data = await response.json();
        //console.log("response data:", data);
        setPrimaryLearningReports(data);
      } catch (error) {
        console.error("Error fetching primary learning reports:", error);
      }
    };
