  ...
  const handlePrimarySaveScorecard = async () => {
    ...
    try {
      ...
      for (const department of departments) {
        ...
        if (!response.ok) {
          throw new Error(
            `Failed to save primary stakeholder scorecard for department ${department.id}`
          );
        }
        const savedScorecard = await response.json();
        // Only update state for department 1
        if (department.id === selectedDepartmentId) {
          setPrimaryStakeholderScorecard((prevScorecards) => [
            ...prevScorecards,
            savedScorecard,
          ]);
        }
      ...
      {primaryModalOpen && (
        <div className="fixed inset-0 flex items-center justify-center">
          ...
          <div className="bg-white p-8 rounded-lg z-10 h-[50rem] w-[96rem]">
            <div className="flex flex-row">
              <h2 className="text-2xl mb-3 font-semibold">Stakeholder</h2>
              <button
                onClick={handleCloseModal}
                className="ml-[83rem] mt-[-2rem] text-gray-500 hover:text-gray-700"
              >
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  fill="none"
            ...
            {!primaryEditMode && (
              <div className="bg-yellow-100 border border-yellow-400 text-yellow-700 px-4 py-3 rounded-lg mb-8">
                ...
                all departments in stakeholder aspect.
              </div>
            )}
