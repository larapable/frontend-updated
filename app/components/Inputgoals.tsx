...
export default function Inputgoals() {
  ...
  useEffect(() => {
    const fetchLatestData = async () => {
      try {
        const response = await fetch('http://localhost:8080/goals/latest');
        if (response.ok) {
          const data = await response.json();
          setOfficeVision(data.vision || '');
          setValueProposition(data.proposition || '');
  ...
  const handleSave = async (event: React.MouseEvent<HTMLButtonElement, MouseEvent>) => {
    ...
    try {
      ...
      const url = isNew ? "http://localhost:8080/goals/insert" : `http://localhost:8080/goals/update/${goalId}`;
      ...
      const response = await fetch(url, {
        ...
        body: JSON.stringify({
          // Only include id if updating an existing goal
          id: isNew ? undefined : goalId,
          vision: officeVision,
      ...
      // If it's a new goal, set the new goal ID and update the state
      if (isNew) {
