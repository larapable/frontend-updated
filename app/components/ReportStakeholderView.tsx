...
const ReportStakeholderView = () => {
  ...
  const [stakeholderReportFirst, setStakeholderReportFirst] = useState<
    ReportStakeholderView[]
  >([]);

  const [stakeholderReportSecond, setStakeholderReportSecond] = useState<
    ReportStakeholderView[]
  >([]);
  useEffect(() => {
    const getFirstReports = async (department_id: number) => {
      try {
        ...
        setStakeholderReportFirst(firstSemesterReports);
      } catch (error) {
    ...
    const getSecondReports = async (department_id: number) => {
      try {
        ...
        // Filter reports to only include the second semester
        const secondSemesterReports = data.filter(
          (report: ReportStakeholderView) => report.semester === "2nd"
        );

        setStakeholderReportSecond(secondSemesterReports);
      } catch (error) {
        console.error("Error fetching stakeholder reports:", error);
      }
    };
